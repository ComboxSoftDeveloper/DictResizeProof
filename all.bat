...
for %%T in (net8.0 net9.0 net10.0) do (
    for %%R in (switch entries primes timing) do (
        echo   %%T %%R
        dotnet run -c Release -f %%T --no-build -- %%R > "%OUT%\%%R_%%T.txt"
    )
)

rem Многоуровневая компиляция гасится вместе с профилем: иначе часть вызовов
rem выполняется кодом первого уровня, и режим ничего не показывает.
echo.
...
rem Две короткие пробы на одном классе. Нужны, чтобы отделить сбой самого
...
dotnet run -c Release -f net10.0 --no-build -- --filter *FastModBench* noasm > "%OUT%\probe_noasm.txt" 2>&1
set PROBE1=%ERRORLEVEL%
if exist "Bdn\DictResizeProof.log" copy /Y "Bdn\DictResizeProof.log" "%OUT%\probe_noasm.log" >nul
if exist "Bdn\*.log" del /Q "Bdn\*.log"
if not "%PROBE1%"=="0" echo   завершилось с ошибкой, смотри "%OUT%\probe_noasm.txt"

echo.
echo === Проба: один класс с дизассемблером ===
dotnet run -c Release -f net10.0 --no-build -- --filter *FastModBench* > "%OUT%\probe_asm.txt" 2>&1
set PROBE2=%ERRORLEVEL%
if exist "Bdn\DictResizeProof.log" copy /Y "Bdn\DictResizeProof.log" "%OUT%\probe_asm.log" >nul
if exist "Bdn\*.log" del /Q "Bdn\*.log"
...
if exist "Bdn\DictResizeProof.log" copy /Y "Bdn\DictResizeProof.log" "%OUT%\bench.log" >nul
